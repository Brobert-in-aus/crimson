...
pub fn packInputFlags(decoded: InputFlags) u32 {
    ...
    if (decoded.aim_scheme) |scheme| {
        ...
        const raw: u32 = if (scheme < 0)
            aim_scheme_mask
        else
            @as(u32, @intCast(scheme)) & aim_scheme_mask;
...
/// Session facts the VR host recorder must carry into the current replay
/// header. The verifier re-simulates from these values, so they must describe
/// the session that was actually played rather than reconstructed defaults.
...
fn recordingQuestLevel(raw: []const u8) !?QuestLevelCurrentWire {
    if (raw.len == 0) return null;
    const dot = std.mem.indexOfScalar(u8, raw, '.') orelse return error.InvalidHeaderValue;
    if (dot == 0 or dot + 1 >= raw.len) return error.InvalidHeaderValue;
    return .{
        .major = try std.fmt.parseInt(i32, raw[0..dot], 10),
        .minor = try std.fmt.parseInt(i32, raw[dot + 1 ..], 10),
    };
}

/// Encode one host-captured row per simulation tick using the current replay
/// schema, then wrap the msgpack payload in the mandatory `.crd` zstd envelope.
pub fn encodeRecording(
    ...
    if (inputs.len != dt.len) return error.UnsupportedInputShape;
    ...
    const ticks = try allocator.alloc(ReplayTickCurrentWire, inputs.len);
    defer allocator.free(ticks);
    const rows = try allocator.alloc([]ReplayInputWire, inputs.len);
    defer allocator.free(rows);
    var built_rows: usize = 0;
    defer for (rows[0..built_rows]) |row| allocator.free(row);

    // Host recordings currently contain ordered perk traffic only. In format
    // v18 those operations live in the tick prelude, before simulation.
    const preludes = try allocator.alloc(ReplayPreludeCurrentWire, events.len);
    defer allocator.free(preludes);
    var event_index: usize = 0;
    var prelude_count: usize = 0;

    for (inputs, 0..) |tick_inputs, tick_index| {
        const row = try allocator.alloc(ReplayInputWire, tick_inputs.len);
        rows[tick_index] = row;
        built_rows += 1;
        for (tick_inputs, 0..) |player, player_index| {
            row[player_index] = .{
        ...
        const prelude_start = prelude_count;
        while (event_index < events.len and events[event_index].tickIndex() == tick_index) {
            preludes[prelude_count] = switch (events[event_index]) {
        ...
        ticks[tick_index] = .{
            .dt = dt[tick_index],
            .inputs = row,
            .prelude = preludes[prelude_start..prelude_count],
            .postlude = &.{},
            .commands = &.{},
        };
    }
    if (event_index != events.len) return error.UnsupportedEventShape;
    ...
    const replay: ReplayCurrentWire = .{
        .header = .{
            ...
            .seed = cfg.seed,
            .replay_format_version = replay_format_version,
            .quest_level = try recordingQuestLevel(cfg.quest_level),
            ...
            .violence_disabled = cfg.gore_disabled,
